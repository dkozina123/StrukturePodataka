...
int main() {
	...
	int kolicina;
	double cijena;
	printf("Unesi prvi datum (god-mj-dan): "); scanf("%s", datum1);
	printf("Unesi drugi datum (god-mj-dan): "); scanf("%s", datum2);
	printf("Unesi trazeni artikl: "); scanf("%s", trazi);
	PronadiArtiklUPeriodu(&head, trazi, datum1, datum2, &kolicina, &cijena);
	printf("Artikla %s je u periodu od %s do %s bilo prodano: %d puta, po cijeni %lf, odnosno sveukupna zarada je bila: %lf.\n", trazi, datum1, datum2, kolicina, cijena, (double)kolicina * cijena);
